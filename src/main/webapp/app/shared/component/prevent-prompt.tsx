...
class ModalConfirm extends Component<IUserConfimProps, IUserConfimState> {
  constructor(props) {
    super(props);
    this.state = {
      showModal: true
        ...
        <Modal.Header>
          <div className="d-flex">
            <div className="header-content">
              <h5 className="tx-18 tx-sftext-semibold mg-b-0 lh-2">{title}</h5>
            </div>
          </div>
        </Modal.Header>
        <Modal.Body>
          <div className={`dailog-message-content ${classNameContent || ''}`}>
            <p className="description mg-b-0" dangerouslySetInnerHTML={{ __html: contentMessage }} />
          </div>
        </Modal.Body>
...
const userConfirmation = (message, callback) => {
  ...
  ReactDOM.render(
    <ModalConfirm reverse message={message} onCancel={() => closeModal(false)} onConfirm={() => closeModal(true)} />,
    container
  );
};

export { ModalConfirm, userConfirmation };
